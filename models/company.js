...
class Company {
   ...
   * 
   * Should find any company whose name contains the query string
   */
...
static async filterByName(name) {
  const result = await db.query(
    ...
    if (companies.length === 0) {
      throw new NotFoundError(`No companies with name similar to: ${name}`);
    }
   ...
   static async findAll(filters = {}) {
    let query = `SELECT handle, name, num_employees AS "numEmployees", description, logo_url AS "logoUrl"
                 FROM companies`;
